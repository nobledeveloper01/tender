...
REQUIRED="README.md DESIGN.md CHANGELOG.md PHASE
          ...
          docs/FEATURE-BACKLOG.md docs/RELEASE-GATES.md docs/DATASET-GUIDE.md docs/APPSTORE.md docs/SPEECH.md docs/RECORDING-KIT.md docs/mark.png"
...
if [ -d .git ]; then
  ...
  if [ -d docs/screenshots ]; then
    ...
    #
    # A warning, not a failure. Most commits under Tender/ change nothing you
    ...
    # go unmentioned.
    code=$(git log -1 --format=%ct -- Tender TenderDomain/Sources 2>/dev/null || echo 0)
    shots=$(git log -1 --format=%ct -- docs/screenshots 2>/dev/null || echo 0)
    if [ "${code:-0}" -gt "${shots:-0}" ]; then
      behind=$(git rev-list --count "$(git log -1 --format=%H -- docs/screenshots)"..HEAD -- Tender TenderDomain/Sources 2>/dev/null || echo '?')
...
if [ -d .git ] && [ -f docs/JOURNAL.md ]; then
  ...
  last_code=$(git log -1 --format=%ct -- Tender TenderDomain/Sources 2>/dev/null || echo 0)
